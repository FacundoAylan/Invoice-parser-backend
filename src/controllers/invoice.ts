...
const parseSingleInvoice = async (invoiceData: {
  ...
  const totalExcludingTaxes = facturaArg.items.reduce((acc, item) => {
    if (
      (proveedor.includes("COCA") || proveedor.includes("MOET")) &&
      item.ivaPorcentaje > 0
    ) {
      const netoTotal =
        (item.precioUnitario - item.impuestosInternos) /
  ...
  const porcentajeImpIntPenaflor =
    facturaArg.subtotalNeto > 0
      ? facturaArg.impuestosInternosTotal / facturaArg.subtotalNeto
      : 0;

  const itemsProcesados = facturaArg.items.map((item) =>
    parseItem(item, proveedor, porcentajeImpIntPenaflor),
  );
...
const parseGeneratedContent = async ({
  imageBase64,
  mimeType,
}: {
  imageBase64: string;
...
const parseItem = (
  ...
  proveedor: string,
  porcentajeImpIntPenaflor: number,
) => {
